...
export const ThemeOptionsContextController = ({ children }: ThemeOptionsContextControllerProps) => {
  const [isDarkTheme, setDarkTheme] = useState<boolean>(true);

  return <ThemeOptionsContext.Provider value={{ isDarkTheme, setDarkTheme }}>{children}</ThemeOptionsContext.Provider>;
};
